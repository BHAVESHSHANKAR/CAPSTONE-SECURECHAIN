    ...
    <div className="w-full" {...getRootProps()}>
      <motion.div
        ...
        <div className="flex flex-col items-center justify-center">
          <p
            className="relative z-20 font-sans font-bold text-neutral-700 text-base">
            {disabled ? 'Upload in progress...' : 'Upload file'}
          </p>
          <p
            className="relative z-20 font-sans font-normal text-neutral-400 text-base mt-2">
            {disabled ? `${uploadProgress}% complete` : 'Drag or drop your files here or click to upload'}
          </p>
            ...
            {files.length > 0 &&
              files.map((file, idx) => (
                <motion.div
                  ...
                  <div className="flex justify-between w-full items-center gap-4">
                    ...
                    <div className="flex items-center gap-3">
                      <motion.p
                        ...
                        animate={{ opacity: 1 }}
                        layout
                        className="rounded-lg px-2 py-1 w-fit shrink-0 text-sm text-neutral-600 bg-gray-100 shadow-input">
                  ...
                  <div
                    className="flex text-sm md:flex-row flex-col items-start md:items-center w-full mt-2 justify-between text-neutral-600">
                    <motion.p
                      initial={{ opacity: 0 }}
                      animate={{ opacity: 1 }}
                      layout
              ...
              <motion.div
                ...
                {isDragActive ? (
                  <motion.p
                    ...
                    {disabled ? 'Uploading...' : 'Drop it'}
                    {disabled ? (
                      <IconLoader2 className="h-4 w-4 text-neutral-600 animate-spin mt-2" />
                    ) : (
                  ...
                  disabled ? (
                    <IconLoader2 className="h-4 w-4 text-neutral-600 animate-spin" />
                  ) : (
                    <IconUpload className="h-4 w-4 text-neutral-600" />
                  )
                )}
